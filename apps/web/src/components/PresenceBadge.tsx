...
const STYLES: Record<string, { dot: string; pill: string }> = {
  ...
  offline: { dot: 'bg-gray-400', pill: 'border-gray-200 bg-gray-50 text-gray-600' },
}
...
export default function PresenceBadge({ presence, className = '' }: PresenceBadgeProps) {
  ...
  return (
    <span className={`inline-flex items-center gap-2 rounded-full border px-3 py-1 text-sm ${style.pill} ${className}`}>
